 ...
 */
 ...
 metadata {
	definition (name: "Monoprice Z-Wave Plus 4-in-1 Sensor", namespace: "robertvandervoort", author: "Robert Vandervoort") {
	...
	tiles (scale: 2) {
		multiAttributeTile(name:"main", type:"generic", width:6, height:4) {
            ...
            tileAttribute("device.humidity", key: "SECONDARY_CONTROL") {
                attributeState "humidity",label:'RH ${currentValue} %'
            }
		}
		...
		valueTile("illuminance","device.illuminance", width: 2, height: 2) {
            	state "luminosity", label:'Light ${currentValue}%'
			}
...
def zwaveEvent(physicalgraph.zwave.commands.sensormultilevelv5.SensorMultilevelReport cmd) {
	...
	switch (cmd.sensorType) {
		case 1:
			...
			map.value = Math.round(convertTemperatureIfNeeded(cmd.scaledSensorValue, cmdScale, cmd.precision) as float)
