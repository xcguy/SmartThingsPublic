...
preferences {
	...
	section("Log devices...") {
		input "contacts", "capability.contactSensor", title: "Open/close sensors", required: false, multiple: true
		...
		input "lightSensors", "capability.illuminanceMeasurement", title: "Light sensors", required: false, multiple: true		
		...
		input "energies", "capability.energyMeter", title: "Energy Meters", required:false, multiple: true
   		input "waterSofteners", "device.waterSoftenerSensor", title: "Water Softeners", required: false, multiple: true
...
def initialize() {
    ...
    subscribe(waterSofteners, "saltLevel", handleWaterSoftenerEvent)
    subscribe(lightSensors, "illuminance", handleLightSensorEvent)
}
...
def handleWaterSoftenerEvent(evt) {
	sendValue(evt) { it.toString() }
}

def handleLightSensorEvent(evt) {
	sendValue(evt) { it.toString() }
}
